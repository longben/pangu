<?php
	...
	if(empty($username) || empty($password)){
		echo "<SCRIPT LANGUAGE='JavaScript'>alert('用户名或密码为空！');location.replace('index.php');</SCRIPT>";
	}
	...
	if($SafeCode == $_SESSION['osSafeCode']){
		...
		if($arr[0]){
			...
			echo "<SCRIPT LANGUAGE='JavaScript'>alert('用户名或密码错误！');location.replace('index.php');</SCRIPT>";
		}
	}else{
			echo "<SCRIPT LANGUAGE='JavaScript'>alert('验证码错误！');location.replace('index.php');</SCRIPT>";
	}
